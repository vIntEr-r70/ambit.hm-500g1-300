...
{
    if (!ambit::lock_system::load_config(path / "flags.json", lockers_, subsystems_))
        return;
    ...
    for (std::size_t idx = 0; idx < lockers_.size(); ++idx)
    {
        ...
        std::string key = std::format("locker/flags/{}", locker.name);
        eng::sibus::client::config_listener(key, [this, idx](std::string_view json)
        {
            ...
            auto &locker = lockers_[idx];
            locker.lock_by_value = cfg.get<bool>("lock-by", true);
...
void lock_flags::update_locker(std::size_t idx, std::optional<bool> value)
{
    ...
    if (!ll.subsystems.empty() && (*value == ll.lock_by_value))
        result = ambit::lock_status::critical;

    update_locker_status(idx, result);
}

void lock_flags::update_locker_status(std::size_t idx, ambit::lock_status value)
{
    auto &ll = lockers_[idx];
