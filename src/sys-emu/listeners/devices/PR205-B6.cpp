...
namespace devices
{
    ...
    bool PR205_B6::bit_get(std::size_t idx)
    {
        std::bitset<8> bitset(get(0x4001));
        return bitset.test(idx);
    }

    void PR205_B6::bit_set(std::size_t idx, bool value)
    {
        std::bitset<8> bitset(get(0x4001));
    ...
    void PR205_B6::bitset(std::size_t idx, bitset_callback callback)
    {
        callbacks_[idx] = std::move(callback);
    }

    bool PR205_B6::bit_p_get(std::size_t idx)
    {
        std::bitset<8> bitset(get(0x4000));
        return bitset.test(idx);
    }

    void PR205_B6::bit_p_set(std::size_t idx, bool value)
    {
        std::bitset<8> bitset(get(0x4000));
        bitset.set(idx, value);
    ...
    void PR205_B6::changed(std::uint16_t address, std::uint16_t value)
    {
        switch (address)
        {
        case 0x4001:
