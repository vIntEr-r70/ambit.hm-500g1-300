...
{
    if (!read_unit_conf_file(name(), fname))
        return;
    ...
    units_interaction::add_new_unit(host_, port_, units.size());
...
bool unit_node::read_unit_conf_file(std::string_view name, std::filesystem::path const &file_name)
{
    ...
    unit_conf_loader::unit_conf_t unit_conf;
    if (!unit_conf_loader::load(file_name, unit_conf))
    {
        std::println("+{}+", std::string(width - 2, '-'));
        std::println("FAILED");
        return false;
    ...
    host_ = unit_conf.host;
    port_ = unit_conf.port;

    std::ranges::for_each(unit_conf.records, [&](auto record)
    {
